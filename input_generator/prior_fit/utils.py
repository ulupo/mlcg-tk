...
def compute_nl_unique_keys(
        ...
        interaction_types = torch.stack([atom_types[mapping[ii]] for ii in range(order)], dim=0)
        interaction_types_sym = _symmetrise_map[order](interaction_types)

        max_type = interaction_types_sym.max().item() + 1
        multipliers = torch.tensor([max_type ** i for i in range(order)], 
                                   dtype=torch.long, device=interaction_types_sym.device)
